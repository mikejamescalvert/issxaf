...
#Region "Public Methods"
        Public Function CreatePMVendor(ByVal PMVndr As PMVendor) As String
            ...
            Try
                ...
                With taPmVendor
                    ...
                    .UpdateIfExists = 0
                    .UseVendorClass = 1
                    If PMVndr.PrimaryAddressIdOverride > String.Empty Then
                        .VADDCDPR = PMVndr.PrimaryAddressIdOverride
                    Else
                        .VADDCDPR = "PRIMARY"
                    End If
        ...
        End Function
        ''' <summary>
        ''' Creates a manual check and applies it to a voucher/invoice
        ''' </summary>
        ''' <returns></returns>
        Public Function CreatePMCheck(ByVal Check As PMCheck) As String
            Dim PM As New eConnect.Serialization.PMManualCheckType
            Dim taPMManualCheck As New eConnect.Serialization.taPMManualCheck
            Dim strNextVoucher As String
            Try

                If Check.VoucherNumber = String.Empty Then
                    strNextVoucher = CommonLogic.GetDocNumber(Me._mMSGPConnectionString, CommonLogic.DocNumberTypes.PMVoucherNumber)
                    Check.VoucherNumber = strNextVoucher
                End If

                With taPMManualCheck
                    .BACHNUMB = Check.BatchNumber
                    .PMNTNMBR = Check.VoucherNumber
                    .VENDORID = Check.VendorId
                    .DOCNUMBR = Check.DocumentNumber
                    .DOCAMNT = Check.DocumentAmount
                    .DOCDATE = Check.DocumentDate
                    .PYENTTYP = Check.PaymentType
                    .CARDNAME = Check.CardName
                    .CHEKBKID = Check.CheckbookId
                    .TRXDSCRN = Check.Description
                    .USRDEFND1 = Check.UserDefined1
                    .USRDEFND2 = Check.UserDefined2
                    .USRDEFND3 = Check.UserDefined3
                    .USRDEFND4 = Check.UserDefined4
                    .USRDEFND5 = Check.UserDefined5
                    .BatchCHEKBKID = Check.BatchCheckbookId
                End With
                PM.taPMManualCheck = taPMManualCheck
                If CommonLogic.eConnectCreate(Me._mMSGPConnectionString, PM) = True Then
                    Return Check.VoucherNumber
                Else
                    Return String.Empty
                End If
            Catch ex As Exception
            ...
            Try
                ...
                If taPMTrx.CREATEDIST = 0 Then
                    ...
                    For intLoop As Integer = 0 To PMTrx.PMTrxDistributionLines.Count - 1
                        ...
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Avail
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Cash
                        '        taPMTrx.CASHAMNT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Fnchg
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Freight
                        '        taPMTrx.FRTAMNT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Gst
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Misc
                        '        taPMTrx.MSCCHAMT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Other
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Pay
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Purch
                        '        taPMTrx.PRCHAMNT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Round
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Taken
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Taxes
                        '        taPMTrx.TAXAMNT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Trade
                        '        taPMTrx.TRDISAMT = PMTrxDistributionLine.CreditAmount + PMTrxDistributionLine.DebitAmount
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Unit
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Wh
                        '        '?
                        '    Case MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Write
                        '        '?
                        'End Select
                        'If PMTrxDistributionLine.DistributionType = MSGP2018R2eConnectIntegration.PM.PMTrxDistributionLine.DistributionTypes.Avail Then
                        ...
                        With taPMTrxDistributionLines(intLoop)
                            ...
                            If PMTrxDistributionLine.LineSeq IsNot Nothing Then
                                .DSTSQNUM = PMTrxDistributionLine.LineSeq
                            End If
                        End With
                    Next
