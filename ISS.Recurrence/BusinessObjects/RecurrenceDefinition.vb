...
''' <summary>
''' Recurrence object designed to track how often an object should be scheduled.
''' Usage: Call the Update method anytime the object has been used.  The Due property will show if the object needs to be executed.
''' </summary>
...
#Region "Behaviors"
    ...
    Private Sub UpdateNextExecutionDatetime()
        ...
        If RecurrencePattern = 0 And HourlyRecurrence <= 0 Then
            Return
        End If
        If RecurrencePattern = 1 And DailyRecurrence <= 0 Then
            Return
        End If
        If RecurrencePattern = 2 And WeeklyRecurrence <= 0 Then
            Return
        End If
        If RecurrencePattern = 3 And MonthlyRecurrence <= 0 Then
            Return
        End If
        If RecurrencePattern = 4 And YearlyRecurrence <= 0 Then
            Return
        End If
        If RecurrencePattern = 5 And MinutesRecurrence <= 0 Then
            Return
        End If
        Dim dteNextTimeToExecute As Date = TimeToExecute.Value
        Dim dowWeek = GetDaysOfWeekToExecute()
        Dim blnChangedWeekday As Boolean = False
        Dim intDateAdded As Integer = 0
        Dim intFirstDay As Integer = -1
        ...
        Do While dteNextTimeToExecute < Now
            Select Case RecurrencePattern
                ...
                Case RecurrencePatternTypes.Monthly
                    If Me.MonthlyRecurrence = 0 Then
                        TimeToExecute = Nothing
                        Return
                    End If
                    dteNextTimeToExecute = dteNextTimeToExecute.AddMonths(Me.MonthlyRecurrence)
                    While IsDayAllowed(dteNextTimeToExecute) = False
                        dteNextTimeToExecute = dteNextTimeToExecute.AddDays(1)
                    End While
                Case RecurrencePatternTypes.Yearly
                    If Me.YearlyRecurrence = 0 Then
                        TimeToExecute = Nothing
                        Return
                    End If
                ...
                Case RecurrencePatternTypes.Weekly
                    If Me.WeeklyRecurrence = 0 Then
                        TimeToExecute = Nothing
                        Return
                    End If

                    'check if there is another day in the week coming up that we should run the notification, set it if so.
                    If dteNextTimeToExecute.DayOfWeek < DayOfWeek.Saturday Then
                        For intLoop As Integer = dteNextTimeToExecute.DayOfWeek + 1 To DayOfWeek.Saturday
                            intDateAdded += 1
                            If IsDayAllowed(dteNextTimeToExecute.AddDays(intDateAdded)) = True And blnChangedWeekday = False Then
                                blnChangedWeekday = True
                                dteNextTimeToExecute = dteNextTimeToExecute.AddDays(intDateAdded)
                            End If
                        Next
                    End If

                    'if we did not change the week day, add the weekly recurrence and set the day of the week to the first day available
                    If blnChangedWeekday = False Then
                        dteNextTimeToExecute = dteNextTimeToExecute.AddDays(Me.WeeklyRecurrence * 7)

                        intFirstDay = GetFirstDayOfWeekToRun()

                        If intFirstDay >= 0 Then

                            dteNextTimeToExecute = dteNextTimeToExecute.AddDays(intFirstDay - dteNextTimeToExecute.DayOfWeek)
    ...
    Public Function GetFirstDayOfWeekToRun() As Integer


        If WeeklyOnSunday = True Then
            Return 0
        ElseIf WeeklyOnMonday = True Then
            Return 1
        ElseIf WeeklyOnTuesday = True Then
            Return 2
        ElseIf WeeklyOnWednesday = True Then
            Return 3
        ElseIf WeeklyOnThursday = True Then
            Return 4
        ElseIf WeeklyOnFriday = True Then
            Return 5
        ElseIf WeeklyOnSaturday = True Then
            Return 6
        Else
            Return -1
        End If
    ...
    Public Function IsDayAllowed(ByVal DateToCheck As DateTime) As Boolean
        If WeeklyOnMonday = True Or
            WeeklyOnFriday = True Or
            WeeklyOnSaturday = True Or
            WeeklyOnSunday = True Or
            WeeklyOnThursday = True Or WeeklyOnTuesday = True Or WeeklyOnWednesday = True Then

            If DateToCheck.DayOfWeek = DayOfWeek.Friday AndAlso WeeklyOnFriday = False Then
                Return False
            End If

            If DateToCheck.DayOfWeek = DayOfWeek.Monday AndAlso WeeklyOnMonday = False Then
                Return False
            End If

            If DateToCheck.DayOfWeek = DayOfWeek.Tuesday AndAlso WeeklyOnTuesday = False Then
                Return False
            End If

            If DateToCheck.DayOfWeek = DayOfWeek.Wednesday AndAlso WeeklyOnWednesday = False Then
                Return False
            End If

            If DateToCheck.DayOfWeek = DayOfWeek.Thursday AndAlso WeeklyOnThursday = False Then
                Return False
            End If

            If DateToCheck.DayOfWeek = DayOfWeek.Saturday AndAlso WeeklyOnSaturday = False Then
                Return False
            End If
            If DateToCheck.DayOfWeek = DayOfWeek.Sunday AndAlso WeeklyOnSunday = False Then
                Return False
            End If
